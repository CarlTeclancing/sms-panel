    ...
    <div class="flex-1 min-h-0 overflow-y-auto">
        <div class="bg-white border-b border-slate-200 px-6 py-4 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 sticky top-0 z-10">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h2 class="text-2xl font-semibold">Boosting Services</h2>
                    <p class="text-sm text-slate-500">Manage Peakerr social media services</p>
                </div>
            </div>
        ...
        <div class="p-6">
            <section class="bg-white border border-slate-200 rounded-lg p-5">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-lg font-semibold">Peakerr Services</h3>
                        <p class="text-sm text-slate-500">Sync and review boosting services.</p>
                    </div>
                    <form method="post" action="<?= url('/admin/boosting-services/sync') ?>">
                        <input type="hidden" name="csrf" value="<?= csrf_token() ?>">
                        <button class="border border-slate-300 px-3 py-2 rounded text-sm">Sync from Peakerr</button>
                    </form>
                </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            <?php if (empty($services)): ?>
                                <tr>
                                    <td colspan="8" class="p-6 text-center text-slate-500">No boosting services found.</td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($services as $service): ?>
                                    <tr class="border-b border-slate-100">
                                        ...
                                        <td class="p-3"><?= (int)$service['cancel'] === 1 ? 'Yes' : 'No' ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </tbody>
